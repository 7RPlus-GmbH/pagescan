...
- Legacy ONNX weights (DocAligner SA24 / LCNet + a DeepLabV3 doc-segmentation net) download from DocsaidLab's Google Drive (the original DocAligner upstream) as before, now with a transparent fallback to the [`7rplus/pagescan-weights`](https://huggingface.co/7rplus/pagescan-weights) Hugging Face mirror if Drive is unreachable.
