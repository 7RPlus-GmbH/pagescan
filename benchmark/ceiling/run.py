"""Cascade ceiling: GT-bbox -> HQ-SAM ViT-B -> mask -> quad.

Stands in for the eventual YOLO output by feeding HQ-SAM the axis-aligned
bbox of the ground-truth quad. Reports the *upper bound* of the cascade —
what performance looks like with a perfect detector. Used as a development
reference; the production path is benchmark/cascade/run.py.
...
MODEL_TYPE = "vit_b"
CHECKPOINT = REPO / "data" / "model" / "sam_hq_vit_b.pth"
...
def main() -> None:
    ...
    print(f"running {len(paths)} images (cascade ceiling: GT bbox -> HQ-SAM)")
    ...
    for p in paths:
        ...
        bbox = quad_to_bbox(gt, w, h, BBOX_DILATE_FRAC)
        ...
        with torch.no_grad():
            masks, scores, _ = predictor.predict(
                box=bbox,
                multimask_output=False,  # box prompt is unambiguous, single mask
                hq_token_only=True,  # use HQ token exclusively for sharpest boundaries
            )
        ...
        print(
            ...
            f"fit={fit_method:18s} area={mask_area_frac:.2f} "
            f"embed={embed_ms:5.0f}ms predict={predict_ms:5.0f}ms"
        )
    ...
    rotated = [r for r in rows if r["rotation_offset"] != 0]
    max_pxs = [r["max_corner_px"] for r in rows]
    print()
    ...
    print(f"rotation mismatches:     {len(rotated)}/{len(rows)} (offset != 0)")
    if rotated:
