...
def _conservative_crop(image: np.ndarray, config: ScanConfig) -> np.ndarray:
    """Crop background edges without ML perspective transform.

    Strategy order (first success wins):
    1. Paper-mask quad — best for folded letters and large documents.
       Bridges fold lines via aggressive morphology, immune to internal edges.
    2. DeepLabV3 segmentation — pixel-level document mask, handles unusual
       backgrounds where paper-mask and edge detection struggle.
    3. Edge-based contour quad — best for small documents on clean backgrounds.
    4. HSV paper bounding box — axis-aligned fallback.
    5. Edge-based bounding box — last resort.

    Conservative approach: prefers to leave some background rather than
    risk cutting document content.
    """
    h, w = image.shape[:2]

    # Step 1: Paper-mask quad (handles folded letters, large docs on any background)
    # No upper coverage limit — for folded letters filling the frame, coverage ≈ 1.0
    # is correct, and the perspective transform still corrects camera tilt.
    paper_corners = detect_paper_quad(image, config)
    if paper_corners is not None:
        p_ordered = order_corners(paper_corners)
        p_area = cv2.contourArea(p_ordered)
        p_coverage = p_area / (h * w)
        if p_coverage > 0.15:
            logger.info(f"  Paper quad perspective (coverage={p_coverage:.2f})")
            return perspective_transform(image, paper_corners)

    # Step 2: DeepLabV3 segmentation (pixel-level mask, different approach)
    seg_corners = detect_corners_segmentation(image)
    if seg_corners is not None:
        s_ordered = order_corners(seg_corners)
        s_area = cv2.contourArea(s_ordered)
        s_coverage = s_area / (h * w)
        if 0.10 < s_coverage < 0.95:
            logger.info(f"  DeepLabV3 segmentation (coverage={s_coverage:.2f})")
            return perspective_transform(image, seg_corners)

    # Step 3: Edge-based contour quad (good for tilted docs with clear edges)
